...
- 以 375px 宽度作为 iPhone 竖屏适配基准。
- 将小屏内容区左右边距从 16px 缩减到 8px。
- 将移动端菜单与常用按钮的最小触摸高度调整为 44px。
- 修复窄屏下网络接口操作按钮可能被裁切的问题。

## v1.0.6 [ 2026.09.02 ]

- 删除无引用图标和旧 WOFF 字体文件。
- 使用原生 Web Animations API 替换自建菜单动画管理器。
- 普通页面与登录页面共用主题 head 配置。
- 合并重复的发布文档。
- 使用 ipk/apk matrix 替代重复的 SDK 构建任务。
...
- 修复弹窗通知继承全局固定定位后脱离弹窗的问题。
- 使用 Flexbox 将弹窗稳定居中。

## v1.0.2 [ 2026.04.01 ]

- 修复会话过期弹窗的对齐、尺寸及长文本换行问题。
- 修复深色模式下防火墙和进程表格出现浅色行的问题。
